...
class ResearcherExtractionDistribuitionComponent extends React.Component {
    state = {
        searchText: '',
        searchedColumn: '',
    };
    ...
    render() {
        ...
        return (
            <Row>
                <Col span={24}>
                    <h3>Studies to {this.props.researcher.name} Extraction Step</h3><hr />
                    <Table bordered columns={columns} dataSource={data}
                        pagination={{ pageSize: 50 }} scroll={{ x: 1300, y: 500 }} />
                    <ExcelFile filename={"extraction-study-researcher-distribuition-" + sufixFileName()} element={<Button type="primary" size={"small"}>Export to XLSX</Button>}>
                        <ExcelSheet dataSet={multiDataSelectionDistribuitionXLSX} name="Organization" />
                    </ExcelFile>&nbsp;
                    <Button type="primary" size={"small"}>
                        <CSVLink filename={"extraction-study-researcher-distribuition-" + sufixFileName() + ".csv"} data={distribuitionCSV}>
                            Export to CSV
                        </CSVLink>
                    </Button>
                </Col>
...
export default ResearcherExtractionDistribuitionComponent;