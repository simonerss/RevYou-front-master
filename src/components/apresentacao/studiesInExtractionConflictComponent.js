...
class StudiesInExtractionConflictComponent extends React.Component {
    ...
    render() {
        ...
        const columns = [
            ...
            { title: 'Extraction Result Status', dataIndex: 'answer_status', key: 'answer_status', width: 150, ...this.getColumnSearchProps('answer_status') }
        ...
        //--------------FOR EXPORT XLSX-------------------------------
        const extractionConflictXLSX = data.map(data => {
            return [
                ...
                { value: ((data.answer_status != null) ? data.answer_status : "") },
            ];
        });
        const multiDataextractionConflictXLSX = [{
            columns: [
                { title: "Title", width: { wpx: 300 } },
                { title: "Authors", width: { wpx: 300 } },
                { title: "Keywords", width: { wpx: 250 } },
                { title: "Year", width: { wpx: 250 } },
                { title: "URL", width: { wpx: 250 } },
                { title: "Status", width: { wpx: 250 } },
            ],
            data: extractionConflictXLSX
        }];

        //-------------FORM CSV EXPORTS:----------------------------
        const extractionConflictCSV = data.map(data => {
            return [
                data.title, data.authors, data.keywords,
                data.year, data.url, data.answer_status
            ]
        });
        extractionConflictCSV.unshift([
        ...
        return (
            <Row>
                <Col span={24}>
                    ...
                    <ExcelFile filename={"extraction-conflict-report-" + sufixFileName()} element={<Button type="primary" size={"small"}>Export to XLSX</Button>}>
                        <ExcelSheet dataSet={multiDataextractionConflictXLSX} name="Organization" />
                    </ExcelFile>&nbsp;
                            <Button type="primary" size={"small"}>
                        <CSVLink filename={"extraction-conflict-report-" + sufixFileName() + ".csv"} data={extractionConflictCSV}>
                            Export to CSV
                                </CSVLink>
                    </Button>
                </Col>
...
export default StudiesInExtractionConflictComponent;