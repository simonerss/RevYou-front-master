...
class ResearcherStudiesExtractionListComponent extends React.Component {
    state = {
        searchText: '',
        searchedColumn: '',
    };
    ...
    render() {
        ...
        const columns = [
            ...
            // { title: 'Article ID ', dataIndex: 'key', key: 'key', width: 150, ...this.getColumnSearchProps('key') },
            {
                ...
                render: (text, record) => {
                    return (
                        <span>
                            <Link to={`/dataExtractionDetails/${record.key}/${record.study_id}/${record.ResearcherId}/${record.TemplateFormId}`}>
        ...
        return (
            <Row>
                <Col span={24}>
                    <h3>Studies Assigned to {this.props.researcher.name} Extraction Step</h3><hr />
...
export default ResearcherStudiesExtractionListComponent;